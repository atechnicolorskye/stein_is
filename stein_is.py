...
def initialise_variables(mu, sigma, n_leaders, n_followers):
    ...
    followers = tf.reshape(init_distribution.sample(n_followers), [n_followers, dim])
    q_density = init_distribution.log_prob(followers)
    leaders = tf.reshape(init_distribution.sample(n_leaders), [n_leaders, dim])
...
class SteinIS(object):
    ...
    def construct_map(self):
        ...
        with tf.variable_scope('k_A_A'):
            ...
            self.A_Squared = tf.reduce_sum(tf.square(self.A), keep_dims=True, axis=1) # 100 x 1
            ...
            self.h = h_num / h_dem
            self.k_A_A = tf.exp(-A_A_Distance_Squared / self.h)
        ...
        with tf.variable_scope('sum_grad_A_k_A_A'):    
            self.sum_grad_A_k_A_A = tf.squeeze(tf.stack([tf.matmul(tf.reshape(self.k_A_A[:, i], (1, -1)), -2 * (self.A - self.A[i]) / self.h) for i in range(self.n_leaders)]))
    ...
    def apply_map(self):
        ...
        with tf.variable_scope('sum_grad_A_k_A_B'):
            self.sum_grad_A_k_A_B = tf.squeeze(tf.stack([tf.matmul(tf.reshape(self.k_A_B[:, i], (1, -1)), -2 * (self.A - self.B[i]) / self.h) for i in range(self.n_followers)]))
        return self.k_A_B, self.sum_grad_A_k_A_B
    ...
    def svgd_update(self):
        ...
        with tf.variable_scope('grad_B_phi_B'):
            ...
            for i in range(self.n_followers):
                ...
                term_1 = tf.reduce_sum(self.k_A_B[:, i]) / (self.h * self.n_leaders) * tf.eye(self.dim, dtype=tf.float64)
                term_2 = tf.matmul(tf.transpose(x), tf.matmul(tf.diag(self.k_A_B[:, i]), x))
                sum_grad_B_grad_A_k_A_B.append(4 * (term_1 - term_2))
            sum_d_log_pA_T_grad_A_k_A_B = tf.stack([tf.matmul(tf.transpose(d_log_pA), tf.matmul(tf.diag(self.k_A_B[:, i]), 2 * (self.A - self.B[i]) / self.h)) for i in range(self.n_followers)])
...
with tf.Session() as sess:
    for i in range(1, iterations+1):
        ...
        # pdb.set_trace()
        A, B, q_density = sess.run([model.n_A, model.n_B, model.n_q_density],
                                   feed_dict={model.A: A, model.B: B, model.q_density: q_density, model.step_size: step_size})
        if i % 100 == 0:
